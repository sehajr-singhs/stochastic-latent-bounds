"""exp13 — robotics domain: SARCOS 7-DoF arm (real measured trajectories).

The robotics entry of the campaign: the SARCOS inverse-dynamics dataset
(44,484 samples of a 7-DoF Sarcos arm; 21 kinematic inputs + 7 joint torques).
Consecutive rows form a smooth measured trajectory (consecutive-row deltas are
~23x smaller than shuffled-row deltas), so the torque channels are treated as
the state of a 7-dimensional stochastic plant, era-split for the gate test:
first 60% = nominal, final 25% = disturbed (guard band between, so first
differences never cross eras). The identification stage measures the honest
regime shift ||dA||; if the eras are dynamically similar, that is the reported
result — this experiment's claim is pipeline generality on real robotics data,
not a manufactured drama.

Protocol identical to exp6/exp11/exp12: per-era linear-Gaussian identification,
certificate trained on the nominal-era plant, factor-vs-full certification at
the standard budgets, era separation probes, and the sound shadow gate with the
disturbed-era plant as the real drift.

Usage:  python experiments/exp13_robotics.py [--quick]
Output: results/exp13_robotics.json (+ checkpoint results/exp13_ckpt.pt)
"""
...
from sbounds.realsys import SensorSystem, load_sarcos
...
CKPT = os.path.join(RESULTS, "exp13_ckpt.pt")
ALPHA, KAPPA, TOL = 0.05, 1.0, 0.05
D_ETA = 2
REGION_SCALE = 0.5   # noise-calibrated region (same reasoning as exp6/exp11)
...
def main() -> None:
    ...
    # ---- 1. identify ---------------------------------------------------------
    data_real = load_sarcos()
    seg = data_real["unit"]
    sys_lo = SensorSystem.fit(data_real["X"][data_real["era_lo"] == 1.0],
                              seg[data_real["era_lo"] == 1.0])
    sys_hi = SensorSystem.fit(data_real["X"][data_real["era_hi"] == 1.0],
                              seg[data_real["era_hi"] == 1.0])
    ident = {
        "n_rows": int(data_real["n_rows"]),
        "D": int(data_real["D"]),
        "kin_sd_ratio": float(data_real["kin_sd_ratio"]),
    ...
    if trained is None:
        ...
        # world-model score: simulated push-forward AND real held-out timeline
    ...
    # ---- 4. era separation: nominal plant vs the disturbed regime ------------
    ...
    # ---- 5. gated hot-swap with the disturbed regime as the real drift -------
    ...
    # ---- results --------------------------------------------------------------
    out = {"dataset": "SARCOS 7-DoF robot arm, inverse dynamics "
                      "(44484 measured samples; torques as state, era-split)",
    ...
    path = os.path.join(RESULTS, "exp13_robotics.json")
    with open(path, "w", encoding="utf-8") as fh:
        json.dump(out, fh, indent=2)
    print("saved results/exp13_robotics.json", flush=True)
